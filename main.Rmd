...
if (exe_plots) {
  ...
  p.fig1b = ggplot_timeseries_one_stock(df[df[[name_id_timeseries]] == this_id, ], variables, 
  ...
  if (save_plots) {
    for (typ in types_plots) {
      ggsave(p.fig1a, filename = paste0(dir_out, typ, "/fig1a-timeseries_durations.", typ), device = typ,
             width = 6, height = 8)
      ggsave(p.fig1b, filename = paste0(dir_out, typ, "/fig1b-timeseries_one_stock-", this_id,".", typ), device = typ,
             width = 5, height = 3)
    }
  }
...
if (exe_plots) {
  ...
  p.figS1 = ggplot_simplex_rho_E(res_simplex_tp1, res_simplex_tp1_summary_opti_E, this_id, this_var, labels_of_variables)
  ...
  if (save_plots) {
    for (typ in types_plots) {
      ggsave(p.figS1, filename = paste0(dir_out, typ, "/figS1-simplex_rho_E-", this_id, ".", typ), device = typ,
             width = 6, height = 4)
  ...
  if (save_plots) {
    for (typ in types_plots) {
      ...
      ggsave(p.figS3b, filename = paste0(dir_out, typ, "/figS3b-convergence_ccm_one_dir-", 
             ...
             device = typ, width = 5, height = 4)
      ggsave(p.fig2a, filename = paste0(dir_out, typ, "/fig2a-convergence_ccm_two_dir-", this_id, ".", typ), 
             device = typ, width = 5, height = 4)
...
ggplot(res_ccm_best_E_assessment, aes(x = max_median_rho_ccm_with_libs, fill = causality)) + 
  facet_wrap(~ var_lib + var_target, scales = "free") + 
